	...
	this.fetchCubeletInfo = function(cubelets) {
		...
		request.get({
			...
			if (response.statusCode != 200) {
				//service.emit('error', new Error('Bad response.'));
				return;
			}
			if (!__(body).isArray() || body.length != cubelets.length) {
				//service.emit('error', new Error('Bad response body.'));
				return;
			}
			for (var i = 0; i < body.length; ++i) {
				var data = body[i];
				var cubelet = cubelets[i];
				service.emit('info', new Info(data), cubelet);
			}
		});
	};
